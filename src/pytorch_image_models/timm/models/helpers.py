...
def load_pretrained(model, default_cfg=None, num_classes=1000, in_chans=3, filter_fn=None, strict=True, progress=False, model_dir=None):
    ...
    if hf_hub_id and has_hf_hub(necessary=not pretrained_url):
        ...
        state_dict = load_state_dict_from_url(pretrained_url, progress=progress, map_location='cpu', file_name=model_dir)
    if filter_fn is not None:
